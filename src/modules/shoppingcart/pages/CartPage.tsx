    ...
    <div className="p-4 sm:p-6 md:p-10 flex flex-col md:flex-row gap-5 min-h-screen pb-32">
              ...
              <div className="flex flex-col gap-4">
                {cart.map((item) => (
                  <ProductCartCard
                    key={item.variant_id}
                    {...item}
                    onIncrement={() => incrementItem(item.variant_id)}
                    onDecrement={() => decrementItem(item.variant_id)}
      ...
      <div className="w-full md:w-1/2">
        ...
        <div className="px-2 py-5 flex flex-col gap-10">
          <div>
            <h1 className="font-work text-gray-500 text-md py-2">Coupouns</h1>
            <DiscountButton onApply={(d) => setDiscount(d)} />
            <div className="w-full flex py-4 flex-row justify-between">
              <h1 className="font-work text-gray-500 text-md">Sub Total</h1>
              <h1 className="w-1/4 flex justify-center text-black">{formatMoney(subtotal)}</h1>
            </div>
            <div className="w-full flex py-2 flex-row justify-between">
              <h1 className="font-work text-gray-500 text-md">Discount</h1>
              <h1 className="w-1/4 flex justify-center text-black">{formatMoney(discountAmount)}</h1>
            </div>
            <div className="w-full flex py-2 flex-row justify-between">
              <h1 className="font-work text-gray-500 text-md">Delivery Fee</h1>
              <h1 className="w-1/4 flex justify-center text-black">{formatMoney(DELIVERY_FEE)}</h1>
            </div>
          </div>
          <div>
            <div className="h-0.5 w-full bg-[#78534A]/10"></div>
            <div className="w-full flex py-4 flex-row justify-between">
              <h1 className="font-work text-gray-500 text-md">Total</h1>
